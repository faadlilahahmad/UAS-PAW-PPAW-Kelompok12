 ...
 */
 if (!$connection = new Mysqli("localhost", "root", "", "pinjam")) {
...
// Update otomatis
$query = $connection->query("SELECT a.id_fasilitas, a.id_transaksi, (DATEDIFF(NOW(), a.tgl_ambil)) AS tgl FROM transaksi a WHERE a.status='0'");
while ($data = $query->fetch_assoc()) {
  if ($data["tgl"] >= 0) {
    $connection->query("UPDATE fasilitas SET status='0' WHERE id_fasilitas=$data[id_fasilitas]");
...
// Pembatalan otomatis
$query = $connection->query("SELECT a.jatuh_tempo, a.id_transaksi, a.id_fasilitas, (TIMESTAMPDIFF(HOUR, a.tgl_sewa, NOW())) AS tempo FROM transaksi a WHERE a.konfirmasi='0'");
while ($data = $query->fetch_assoc()) {
  if ($data["tempo"] > 3) {
    $connection->query("UPDATE transaksi SET pembatalan='1' WHERE id_transaksi=$data[id_transaksi]");
    $connection->query("UPDATE fasilitas SET status='1' WHERE id_fasilitas=$data[id_fasilitas]");
    $q = $connection->query("SELECT id_supir FROM detail_transaksi WHERE id_transaksi=$data[id_transaksi]");
    if ($q->num_rows) {
