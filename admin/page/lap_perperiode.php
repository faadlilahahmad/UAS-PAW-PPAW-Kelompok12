...
if (isset($_GET["action"])) {
	...
	if ($_GET["action"] == "ambil") {
		...
		$connection->query("UPDATE fasilitas SET status='1' WHERE id_fasilitas=".$r["id_fasilitas"]);
	...
	<div class="panel panel-info">
		...
		<div class="panel-body">
				<table class="table table-condensed">
						...
						<tbody>
								<?php $no = 1; ?>
								<?php if ($query = $connection->query("SELECT * FROM transaksi t JOIN fasilitas m USING(id_fasilitas) JOIN pelanggan p ON t.id_pelanggan=p.id_pelanggan WHERE t.tgl_sewa BETWEEN '$_POST[start]' AND '$_POST[stop]'")): ?>
