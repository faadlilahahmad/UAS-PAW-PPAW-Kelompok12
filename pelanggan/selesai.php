...
// Validasi
$sql = $connection->query("SELECT a.id_fasilitas, a.tgl_ambil, a.lama FROM transaksi a WHERE a.id_fasilitas=$_POST[id_fasilitas] AND a.status='0'");
if ($sql->num_rows) {
    $d = $sql->fetch_assoc();
    $sql = "SELECT
      (SELECT ((
        DATEDIFF(ADDDATE('$tgl_ambil', INTERVAL $_POST[lama] DAY), ADDDATE('$d[tgl_ambil]', INTERVAL $d[lama] DAY))
      )) FROM transaksi WHERE id_fasilitas=$d[id_fasilitas] LIMIT 1) AS a,
      (SELECT ((
        DATEDIFF(ADDDATE('$d[tgl_ambil]', INTERVAL $d[lama] DAY), ADDDATE('$tgl_ambil', INTERVAL $_POST[lama] DAY))
      )) FROM transaksi WHERE id_fasilitas=$d[id_fasilitas] LIMIT 1) AS b";
    $s = $connection->query($sql);
    $a = $s->fetch_assoc();
    if ($a["a"] == 0 AND $a["b"] == 0) {
        echo alert("Maaf, fasilitas yang anda sewa sudah di pesan!");
        exit;
    }
}
$query = $connection->query("SELECT * FROM fasilitas WHERE id_fasilitas=$_POST[id_fasilitas]");
...
$connection->query("INSERT INTO transaksi VALUES (NULL, $id, $_POST[id_fasilitas], '$now', '$tgl_ambil', NULL, $_POST[lama], $totalbayar, '0', '$_POST[jaminan]', NULL, '$jatuhtempo', '0', '0')");
...
<div class="panel panel-info">
    ...
    <div class="panel-body">
        ...
        <p>
            ...
            <strong>(0986-01-025805-53-8 a/n SEWA fasilitas)</strong> untuk menyelesaikan pembayaran. dan untuk uang muka minimal setengah dari harga sewa.
        </p>
        <p>
